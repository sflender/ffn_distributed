...
#--my own flags below
flags.DEFINE_integer('summary_every_steps', 60,
...
flags.DEFINE_float('decay_learning_rate_fraction',0,"fraction for decay - set to 0 for no deacy")
flags.DEFINE_float('decay_learning_rate_steps',10000,"steps for decay")
...
def run_training_step(sess, model, fetch_summary, feed_dict):
  ...
  ops_to_run = [model.train_op, model.global_step, model.logits, model.loss]
  ...
  # results[3] contains the loss on this rank
  #print( "loss for this step on rank ",hvd.rank()," : ", results[3] )
  ...
  return prediction, step, summ, results[3]
...
def train_ffn(model_cls, **model_kwargs):
  with tf.Graph().as_default():
    ...
    eval_shape_zyx = train_eval_size(model).tolist()[::-1] # size of the subvolume (within which the FOV moves)
    eval_tracker = EvalTracker(eval_shape_zyx) # computes summary statistics inside EFOV
    load_data_ops = define_data_input(model, queue_batch=1) # this creates a batch of training subvolumes
    prepare_ffn(model) # here the tf graph is defined
    merge_summaries_op = tf.summary.merge_all() # merges all summaries defined in the graph.
    ...
    if FLAGS.shuffle_moves:
      random.shuffle(fov_shifts) #--this will shuffle the FOV positions that make up an extended FOV (EFOV)
    ...
    while step < FLAGS.max_steps:
      ...
      if (FLAGS.scaling_rule>0): #--scale the learning rate linearly (1) or sqrt (2)
        if FLAGS.scaled_lr==1:
          scaled_lr *= hvd.size()
        elif FLAGS.scaled_lr==2:
          scaled_lr *= np.sqrt(hvd.size())
        if step<FLAGS.warmup_steps:
          scaled_lr = FLAGS.learning_rate + (step/float(FLAGS.warmup_steps)) * (scaled_lr-FLAGS.learning_rate)

      if (FLAGS.decay_learning_rate_fraction>0): # constantly decay the learning rate using exponential decay
        scaled_lr *= (FLAGS.decay_learning_rate_fraction)**(step/FLAGS.decay_learning_rate_steps)
      ...
      mask.update_at(seed, (0, 0, 0), updated_seed) # updates the mask inside the subvolume batches
      if hvd.rank()==0:
        this_time = time.time()-time_step_start # how long did this step take
        timing.append(this_time)
        print("step %i took %.2f seconds" %(step-1, this_time) )

      if summ is not None:
        summaries.append(tf.Summary.FromString(summ)) # this adds the summaries from the single FOV
        ...
        if hvd.rank()==0:
          ...
          summ.value.extend( [tf.Summary.Value(tag='avg_time_per_step',simple_value=np.mean(timing) ) ] ) #avg time per step
          print("avg time per step: ", np.mean(timing), np.std(timing))
    ...
    if hvd.rank()==0:
      ...
      if (FLAGS.do_benchmark_test==1):
        ...
        string = str(FLAGS.max_steps)+","+str(FLAGS.batch_size)+","+str(hvd.size())+","+str(FLAGS.nthreads)+","+str(np.mean(timing))+","+str(np.std(timing))+"\n"
        print(string)
